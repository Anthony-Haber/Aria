#!/usr/bin/env python3
"""
MIDI latency and timing calibration tool for the real-time Ableton bridge.

This is a lightweight port of demo/calibrate.py for Windows/loopMIDI setups.
"""
...
def schedule_note_off(port: mido.ports.BaseOutput, note: int, delay: float):
    """Schedule a non-blocking MIDI note-off message."""
...
def strike(
    ...
    """
    Countdown and send MIDI notes with a precise offset.
...
def note_repetition_trial(
    ...
    """Play a note or chord repeatedly for a 3-second trial period."""
...
def velocity_strike_pair(
    ...
    """
    Sends a low-velocity note, then a high-velocity note after a delay.
    Adjust until they sound simultaneous.
    """
    print("Playing velocity pair (C4 high-vel, D4 low-vel)...")
...
def calibrate_output_latency(
    ...
    """Interactive loop to find the ideal output latency offset."""
...
def calibrate_velocity_latency(
    ...
    This mode uses fixed notes and ignores the --chord flag.
    """
    _ = chord_mode
    ...
    try:
        with mido.open_output(port_name) as port:
            ...
            print(
                ...
                f"Low-velocity note  (D4): {low_velocity}\n"
            )
            while True:
...
def measure_input_latency(port_name: str, timeout_sec: float = 2.0):
    """
    Countdown -> you strike a key on GO.
...
def list_midi_ports() -> None:
    """Print a list of available MIDI output ports."""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="A tool to calibrate MIDI latency and note timing.",
...
def main():
    """Dispatch to the selected calibration or measurement routine."""
    args = parse_args()
